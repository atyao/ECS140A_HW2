...
public class Parser {
    ...
    int[] excntLN = new int[101]; // Storing Line Number values for EXCNT to avoid stop skipping. 101 to avoid overflow.
    ...
    private void excnt(){  // EXCNT Function
        ...
        excntLN[exCount] = getLine(); // Record line number for ID exCount
    ...
    private void exTable(){ // Print function for EXCNT
        ...
        for (int x = 0; x < exCount; x++)  // Affix all the Line Number values here to avoid stop skipping
            gcprint("EXCNTarray[" + String.valueOf(x) + "].lineNumber = " + String.valueOf(excntLN[x]) + ";");
    	  gcprint("printf(\"---- Execution Counts ----\\n\");");
    	  gcprint("printf(\"%4s%5s%9s\\n\", \"num\", \"line\", \"count\");");
    	  gcprint("for(int currentEXE = 0; currentEXE < " + String.valueOf(exCount)
    		    + "; currentEXE++) {");
    	  gcprint("printf(\"%4d%5d%9d\\n\", (currentEXE + 1), EXCNTarray[currentEXE].lineNumber, EXCNTarray[currentEXE].count);");
    	  gcprint("}");
