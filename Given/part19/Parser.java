...
public class Parser {
    ...
    ArrayList<Entry> variableList = new ArrayList<>();
    String baffle;
    ...
    private void dump(){
        String msg;
        int dumpLineNumber = tok.lineNumber; // Save the line number so we can scan
        scan();
        int level = this.symtab.get_level();
        if(is(TK.NUM)){
            ...
            gcprint("printf(" + msg + "\");");
            printVariables(Optional.of(dumpLevel));
    ...
    private void printVariables(Optional<Integer> dumpLevel) {
        Set<String> isUsed = new HashSet<>();
        for(Entry variable : this.variableList){
            if(dumpLevel.isPresent() && dumpLevel.get() != variable.getLevel()) continue;
            Entry entry = findEntry(isUsed, variable.getName(), dumpLevel);
            if(entry != null){
    ...
    private Entry findEntry(Set<String> isUsed, String variable, Optional<Integer> startLevel) {
        Entry entry = null;
        if(startLevel.isPresent()){
